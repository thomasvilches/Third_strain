...
dc = [1;map(y->58+y,0:1);map(y->100+y,0:21);map(y->137+y,0:12);map(y->189+y,0:8);map(y->228+y,0:22);map(y->298+y,0:16)]
rc = [1.0;map(y->1.0+(0.1/2)*y,1:2);map(y->1.1-(0.14/22)*y,1:22);map(y->0.96-(0.08/13)*y,1:13);map(y->0.88+(0.045/9)*y,1:9);map(y->0.925-(0.10/23)*y,1:23);map(y->0.825+(0.25/17)*y,1:17)]
...
dc = [1;map(y->137+y,0:19);map(y->200+y,0:9);map(y->220+y,0:25);map(y->298+y,0:16)]
rc = [1.0;map(y->1.0-(0.19/20)*y,1:20);map(y->0.81+(0.06/10)*y,1:10);map(y->0.87-(0.11/26)*y,1:26);map(y->0.76+(0.21/17)*y,1:17)]
...
rc = [1.0;map(y->1.0-(0.217/25)*y,1:25);map(y->0.883+(0.13/17)*y,1:17)]
...
dc = [1;map(y->64+y,0:25);map(y->223+y,0:24);map(y->305+y,0:14)]
rc = [1.0;map(y->1.0-(0.167/26)*y,1:26);map(y->0.833-(0.13/25)*y,1:25);map(y->0.703+(0.17/15)*y,1:15)]
run_param_scen_cal(0.125,"illinois",6,18,1,1,1,1,1,119,160,212,999,143,0.2,1,3,14,rc,dc,365,true)


dc = [1;map(y->57+y,0:2);map(y->65+y,0:14);map(y->118+y,0:4);map(y->217+y,0:24);map(y->292+y,0:14)]
rc = [1.0;map(y->1.0+(0.26/3)*y,1:3);map(y->1.3-(0.42/15)*y,1:15);map(y->0.88+(0.05/5)*y,1:5);map(y->0.93-(0.12/25)*y,1:25);map(y->0.81+(0.16/15)*y,1:15)]
run_param_scen_cal(0.112,"iowa",9,80,1,1,1,1,1,127,207,241,212,205,0.2,1,3,14,rc,dc,365,true)


dc = [1;map(y->56+y,0:2);map(y->74+y,0:14);map(y->142+y,0:6);map(y->175+y,0:36);map(y->225+y,0:24);map(y->302+y,0:14)]
rc = [1.0;map(y->1.0+(0.2/3)*y,1:3);map(y->1.2-(0.35/15)*y,1:15);map(y->0.85-(0.03/7)*y,1:7);map(y->0.82+(0.50/37)*y,1:37);map(y->1.32-(0.32/25)*y,1:25);map(y->1.0+(0.30/15)*y,1:15)]
run_param_scen_cal(0.11,"minnesota",9,60,1,1,1,1,1,67,131,215,163,151,0.2,1,3,14,rc,dc,365,true)



dc = [1;map(y->76+y,0:10);map(y->146+y,0:9);map(y->193+y,0:17);map(y->282+y,0:14)]
rc = [1.0;map(y->1.0-(0.12/11)*y,1:11);map(y->0.88-(0.05/10)*y,1:10);map(y->0.83+(0.15/18)*y,1:18);map(y->0.98+(0.16/15)*y,1:15)]
run_param_scen_cal(0.073,"kansas",3,35,25,1,1,1,1,1,213,233,196,200,0.2,1,3,14,rc,dc,365,true)



dc = [1;map(y->76+y,0:10);map(y->148+y,0:13);map(y->227+y,0:24);map(y->282+y,0:14)]
rc = [1.0;map(y->1.0-(0.14/11)*y,1:11);map(y->0.86+(0.08/14)*y,1:14);map(y->0.94-(0.20/25)*y,1:25);map(y->0.74+(0.21/15)*y,1:15)]
run_param_scen_cal(0.116,"nebraska",6,50,1,1,1,1,1,163,196,245,199,211,0.2,1,3,14,rc,dc,365,true)


dc = [1;map(y->64+y,0:34);map(y->175+y,0:9);map(y->221+y,0:21);map(y->302+y,0:14)]
rc = [1.0;map(y->1.0-(0.229/35)*y,1:35);map(y->0.771+(0.099/10)*y,1:10);map(y->0.87-(0.06/22)*y,1:22);map(y->0.81+(0.11/15)*y,1:15)]
run_param_scen_cal(0.11,"northdakota",1,100,1,1,1,1,1,51,211,252,141,124,0.2,1,3,14,rc,dc,365,true)



dc = [1;map(y->63+y,0:11);map(y->157+y,0:10);map(y->219+y,0:21);map(y->307+y,0:14)]
rc = [1.0;map(y->1.0-(0.13/12)*y,1:12);map(y->0.87+(0.12/11)*y,1:11);map(y->0.99-(0.20/22)*y,1:22);map(y->0.79+(0.33/15)*y,1:15)]
run_param_scen_cal(0.11,"southdakota",1,90,1,1,1,1,1,161,199,280,183,185,0.2,1,3,14,rc,dc,365,true)


###not calibrated
dc = [1;map(y->80+y,0:10);map(y->130+y,0:9);map(y->180+y,0:9);map(y->228+y,0:24);map(y->307+y,0:16)]
rc = [1.0;map(y->1.0-(0.10/11)*y,1:11);map(y->0.9-(0.08/10)*y,1:10);map(y->0.82+(0.05/10)*y,1:10);map(y->0.87-(0.16/25)*y,1:25);map(y->0.71+(0.18/17)*y,1:17)]
run_param_scen_cal(0.112,"indiana",3,30,1,1,1,1,1,119,160,212,999,143,0.2,1,3,14,rc,dc,365,true)
...
run_param_scen_cal(0.114,"nevada",7,30,1,1,1,1,1,154,200,217,999,155,0.2,1,3,14,rc,dc,597,true)
run_param_scen_cal(0.114,"nevada",7,30,1,1,1,1,1,154,200,217,999,155,0.2,2,3,14,rc,dc,597,true,"statuscuo",1.0,0.5)
...
run_param_scen_cal(0.076,"arizona",5,40,8,1,1,1,1,1,177,152,999,186,0.2,1,3,14,rc,dc,597,true)
run_param_scen_cal(0.076,"arizona",5,40,8,1,1,1,1,1,177,152,999,186,0.2,1,3,14,rc,dc,597,true,"statuscuo",1.0,0.5)


dc = [1;map(y->68+y,0:31);map(y->147+y,0:11);map(y->192+y,0:14);map(y->250+y,0:19);map(y->310+y,0:14)]
rc = [1.0;map(y->1.0-(0.18/32)*y,1:32);map(y->0.82-(0.06/12)*y,1:12);map(y->0.76+(0.06/15)*y,1:15);map(y->0.82-(0.06/20)*y,1:20);map(y->0.76+(0.08/15)*y,1:15)]
run_param_scen_cal(0.12,"colorado",4,13,1,1,1,1,1,86,154,233,999,170,0.2,1,3,14,rc,dc,597,true)
run_param_scen_cal(0.12,"colorado",4,13,1,1,1,1,1,86,154,233,999,170,0.2,2,3,14,rc,dc,597,true,"statuscuo",1.0,0.5)


dc = [1;map(y->87+y,0:27);map(y->144+y,0:17);map(y->198+y,0:18);map(y->232+y,0:19);map(y->307+y,0:14)]
rc = [1.0;map(y->1.0-(0.13/28)*y,1:28);map(y->0.87-(0.09/18)*y,1:18);map(y->0.78+(0.10/19)*y,1:19);map(y->0.88-(0.13/20)*y,1:20);map(y->0.75+(0.18/15)*y,1:15)]
run_param_scen_cal(0.106,"oregon",2,13,1,1,1,1,1,105,149,227,999,168,0.2,1,3,14,rc,dc,597,true)
run_param_scen_cal(0.106,"oregon",2,13,1,1,1,1,1,105,149,227,999,168,0.2,2,3,14,rc,dc,597,true,"statuscuo",1.0,0.5)


dc = [1;map(y->92+y,0:17);map(y->170+y,0:9);map(y->217+y,0:21);map(y->295+y,0:16)]
rc = [1.0;map(y->1.0-(0.105/18)*y,1:18);map(y->0.895+(0.02/10)*y,1:12);map(y->0.915-(0.22/22)*y,1:22);map(y->0.695+(0.185/17)*y,1:17)]
run_param_scen_cal(0.107,"idaho",6,80,1,1,1,1,1,129,210,197,999,154,0.2,1,3,14,rc,dc,597,true)
run_param_scen_cal(0.107,"idaho",6,80,1,1,1,1,1,129,210,197,999,154,0.2,1,3,14,rc,dc,597,true,"statuscuo",1.0,0.5)


dc = [1;map(y->73+y,0:25);map(y->235+y,0:41);map(y->298+y,0:16)]
rc = [1.0;map(y->1.0-(0.162/26)*y,1:26);map(y->0.838-(0.09/42)*y,1:42);map(y->0.748+(0.24/17)*y,1:17)]
run_param_scen_cal(0.12,"newmexico",4,15,1,1,1,1,1,125,210,234,179,216,0.2,1,3,14,rc,dc,597,true)
run_param_scen_cal(0.12,"newmexico",4,15,1,1,1,1,1,125,210,234,179,216,0.2,2,3,14,rc,dc,597,true,"statuscuo",1.0,0.5)



dc = [1;map(y->80+y,0:27);map(y->132+y,0:17);map(y->192+y,0:23);map(y->240+y,0:21);map(y->305+y,0:16)]
rc = [1.0;map(y->1.0-(0.12/28)*y,1:28);map(y->0.88-(0.14/18)*y,1:18);map(y->0.74+(0.12/24)*y,1:24);map(y->0.86-(0.132/22)*y,1:22);map(y->0.728+(0.185/17)*y,1:17)]
run_param_scen_cal(0.111,"washington",5,15,1,1,1,1,1,84,176,215,143,151,0.2,1,3,14,rc,dc,597,true)
run_param_scen_cal(0.111,"washington",5,15,1,1,1,1,1,84,176,215,143,151,0.2,2,3,14,rc,dc,597,true,"statuscuo",1.0,0.5)


dc = [1;map(y->65+y,0:4);map(y->108+y,0:29);map(y->194+y,0:18);map(y->287+y,0:24)]
rc = [1.0;map(y->1.0+(0.1/5)*y,1:5);map(y->1.1-(0.225/30)*y,1:30);map(y->0.875+(0.06/19)*y,1:19);map(y->0.935+(0.35/25)*y,1:25)]
run_param_scen_cal(0.068,"california",6,15,15,1,1,15,1,1,178,146,1,128,0.2,1,3,14,rc,dc,365,true)


dc = [1;map(y->74+y,0:13);map(y->141+y,0:9);map(y->214+y,0:21);map(y->310+y,0:16)]
rc = [1.0;map(y->1.0-(0.20/14)*y,1:14);map(y->0.8-(0.07/10)*y,1:10);map(y->0.73+(0.095/22)*y,1:22);map(y->0.825+(0.02/17)*y,1:17)]
run_param_scen_cal(0.116,"wyoming",1,25,1,1,1,1,1,65,210,257,79,149,0.2,1,3,14,rc,dc,365,true)



dc = [1;map(y->75+y,0:11);map(y->118+y,0:12);map(y->246+y,0:17);map(y->308+y,0:16)]
rc = [1.0;map(y->1.0-(0.14/12)*y,1:12);map(y->0.86+(0.08/13)*y,1:13);map(y->0.94-(0.16/18)*y,1:18);map(y->0.79+(0.09/17)*y,1:17)]
run_param_scen_cal(0.105,"montana",1,80,1,1,1,1,1,165,224,231,198,217,0.2,1,3,14,rc,dc,365,true)


dc = [1;map(y->74+y,0:9);map(y->134+y,0:19);map(y->200+y,0:18);map(y->293+y,0:16)]
rc = [1.0;map(y->1.0-(0.07/10)*y,1:10);map(y->0.93-(0.14/20)*y,1:20);map(y->0.79+(0.06/19)*y,1:19);map(y->0.85+(0.012/17)*y,1:17)]
run_param_scen_cal(0.109,"utah",5,70,1,1,1,1,1,86,154,233,999,170,0.2,1,3,14,rc,dc,365,true)
